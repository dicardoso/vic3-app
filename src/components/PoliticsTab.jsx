  ...
  return (
    <div className="space-y-8">
      <div className="mb-8 border-b border-slate-800 pb-8">
        <h2 className="text-3xl font-bold text-white mb-3">{dict.politics.title}</h2>
        <p className="text-slate-400 max-w-4xl leading-relaxed">
          {dict.politics.description}
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4 relative">
        ...
        {politicsNodes.map((node, i) => (
          <React.Fragment key={i}>
            <div
              onClick={() => setSelectedPoliticsNode(node)}
      ...
      <div className="mt-8 p-5 bg-indigo-900/20 border border-indigo-800/30 rounded-xl">
        <h4 className="text-lg font-bold text-indigo-400 flex items-center gap-2 mb-2">
          <ShieldAlert className="w-5 h-5" /> {dict.politics.understandingLegitimacy}
        </h4>
        <p className="text-sm text-indigo-200/80 leading-relaxed">
          {dict.politics.legitimacyDescription}
        </p>
      </div>

      {/* Modal */}
      {selectedPoliticsNode && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm" onClick={() => setSelectedPoliticsNode(null)}>
          <div
          ...
          >
            <div className={`sticky top-0 bg-slate-900/95 backdrop-blur z-10 p-6 border-b border-slate-800 flex justify-between items-start relative overflow-hidden`}>
              <div className="flex items-center gap-4 relative z-10">
                <div className={`p-3 rounded-xl shadow-sm border ${selectedPoliticsNode.themeBg} ${selectedPoliticsNode.themeBorder}`}>
                  {selectedPoliticsNode.icon}
                </div>
                <div>
                  <h2 className="text-2xl font-bold text-white mb-1">{selectedPoliticsNode.title}</h2>
                  <span className={`text-xs uppercase tracking-wider font-semibold ${selectedPoliticsNode.themeColor}`}>
                    {dict.politics.nodeModalSubtitle}
                  </span>
                </div>
              </div>
              <button
                onClick={() => setSelectedPoliticsNode(null)}
            ...
            <div className="p-6 md:p-8 space-y-8">
              <section>
                ...
                <p className="text-lg text-slate-300 leading-relaxed">{selectedPoliticsNode.details}</p>
              </section>
              ...
              <div className="bg-slate-950 p-6 rounded-xl border border-slate-800 shadow-inner grid grid-cols-1 md:grid-cols-2 gap-8 relative">
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    {selectedPoliticsNode.inputs.map((input, idx) => (
                ...
                <div>
                  <div className="flex items-center justify-end md:justify-start gap-2 mb-4">
                    <h4 className={`text-sm font-bold uppercase tracking-widest ${selectedPoliticsNode.themeColor}`}>{dict.politics.generates}</h4>
                  </div>
                  <div className="flex flex-col gap-2">
                    {selectedPoliticsNode.outputs.map((output, idx) => (
                      <div key={idx} className={`bg-slate-800 border px-4 py-2.5 rounded-lg text-white text-sm font-medium shadow-sm flex items-center ${selectedPoliticsNode.themeBorder}`}>
                        <div className={`w-1.5 h-1.5 rounded-full mr-3 ${selectedPoliticsNode.themeBg.replace('/20', '')}`}></div>
                        {output}
                      </div>
                    ))}
                  </div>
